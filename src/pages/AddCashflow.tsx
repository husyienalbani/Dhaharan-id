...
export default function Cashflow() {
  const [cashflow, setCashflow] = useLocalStorage<CashflowItem[]>("cashflow", initialCashflow);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <motion.div
              ...
              className="w-12 h-12 rounded-xl bg-primary border-2 border-foreground shadow-cartoon flex items-center justify-center"
            >
          ...
          <Card className="bg-accent">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-accent-foreground/80">Pemasukan</p>
                  <p className="font-fredoka text-lg md:text-xl font-bold text-accent-foreground truncate">
                    {formatCurrency(totalIncome)}
                  </p>
                </div>
                <TrendingUp className="w-6 h-6 text-accent-foreground/60" />
              </div>
            </CardContent>
          </Card>
        </motion.div>
        ...
        >
          <Card className="bg-highlight">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-highlight-foreground/80">Pengeluaran</p>
                  <p className="font-fredoka text-lg md:text-xl font-bold text-highlight-foreground truncate">
                    {formatCurrency(totalExpense)}
                  </p>
                </div>
                <TrendingDown className="w-6 h-6 text-highlight-foreground/60" />
              </div>
            </CardContent>
          </Card>
        </motion.div>
        ...
        >
          <Card className="bg-secondary">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-secondary-foreground/80">Saldo</p>
                  <p className={`font-fredoka text-lg md:text-xl font-bold truncate ${balance >= 0 ? "text-accent" : "text-highlight"}`}>
                    {formatCurrency(balance)}
                  </p>
                </div>
                <Wallet className="w-6 h-6 text-secondary-foreground/60" />
              </div>
            </CardContent>
          </Card>
        </motion.div>
          ...
          <Card>
            <CardContent className="py-16 text-center">
              ...
              <h3 className="font-fredoka text-lg font-semibold mb-2">
                {searchQuery || filterType !== "all" ? "Tidak Ada Hasil" : "Belum Ada Transaksi"}
              </h3>
